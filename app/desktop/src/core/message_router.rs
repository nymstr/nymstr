...
//! This module provides a clean separation between message classification
...
pub enum MessageRoute {
    ...
    Authentication,
    /// MLS protocol messages (keyPackageRequest, p2pWelcome, etc.)
    MlsProtocol,
...
impl MessageRouter {
    ...
    /// Route a message by action string alone (useful for testing or when envelope not available)
...
mod tests {
    ...
    fn test_route_description() {
        assert_eq!(
            MessageRouter::route_description(&MessageRoute::Authentication),
            "Authentication protocol message"
        );
        assert_eq!(
            MessageRouter::route_description(&MessageRoute::MlsProtocol),
            "MLS protocol message"
        );
        assert_eq!(
            MessageRouter::route_description(&MessageRoute::Unknown),
            "Unknown message type"
        );
    }

    #[test]
    fn test_route_action_direct() {
        assert_eq!(
            MessageRouter::route_action("challenge"),
            MessageRoute::Authentication
        );
        assert_eq!(
            MessageRouter::route_action("keyPackageRequest"),
            MessageRoute::MlsProtocol
        );
        assert_eq!(
            MessageRouter::route_action("mlsWelcome"),
            MessageRoute::WelcomeFlow
        );
        assert_eq!(
            MessageRouter::route_action("unknownAction"),
            MessageRoute::Unknown
        );
    }
}
